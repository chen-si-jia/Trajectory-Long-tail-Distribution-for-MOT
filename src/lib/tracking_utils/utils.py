...
def plot_mot_tracking_online_add_conf(img, online_tlwhs, online_ids, online_scores , frame_id=0, name='MOT', seq_name=None, opt=None):
    """
    ...
    for box, id, score in zip(online_tlwhs, online_ids, online_scores):
        x, y, w, h = int(box[0]), int(box[1]), int(box[2]), int(box[3])
        id = int(id)
        score = float(score)
        score = round(score, 2) # 2位
        ...
        if id not in id_color.keys():
            ...
            cv2.putText(img, str(id), (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 4)
            cv2.putText(img, str(score), (x, y+h-3), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2) # green
            if False:
