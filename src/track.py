...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    result_root = osp.join(opt.output_dir, exp_name, 'result_{}'.format(opt.benchmark_name))
    if not osp.exists(result_root): os.makedirs(result_root)
    vis_video_root = os.path.join(opt.output_dir, exp_name, 'vis/video')
    if not osp.exists(vis_video_root): os.makedirs(vis_video_root)
    vis_img_root = os.path.join(opt.output_dir, exp_name, 'vis/img')
    ...
    for seq in seqs:
        ...
        evaluator = Evaluator(data_root, seq, data_type, opt.gt_txt)
        accs.append(evaluator.eval_file(result_filename))
        if opt.save_videos:
            output_video_path = osp.join(vis_video_root, '{}.mp4'.format(seq))
            cmd_str = 'ffmpeg -f image2 -i {}/%06d.jpg -c:v copy {}'.format(osp.join(vis_img_root, seq), output_video_path)
            os.system(cmd_str)
    ...
    print(strsummary)
    Evaluator.save_summary(summary, os.path.join(opt.output_dir, exp_name, 'summary_MOT.xlsx'))
...
if __name__ == '__main__':
    ...
    exp_name = opt.exp_name if opt.exp_name is not None else opt.benchmark_name
