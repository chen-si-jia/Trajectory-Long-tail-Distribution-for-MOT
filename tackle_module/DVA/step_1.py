...
# 输入：数据集
# 输出：抠图+修复后的图片
# 说明：sequenceName 参数需要修改，需要提前建立好文件夹
...
def SAMTackleBoxes(predictor, backgroundImagePath, originalImagePath, input_boxes_xywh, tx, ty):
    ...
    # img = cv2.imread('temp3.png')     # input
    # mask = cv2.imread('temp2.png',0)  # mask 修复掩码，8位1通道图像。非零像素表示需要修复的区域。

    mask = cv2.cvtColor(translated_image, cv2.COLOR_BGR2GRAY)

    output_image = cv2.inpaint(res1, mask, 3, cv2.INPAINT_NS)
...
if __name__ == '__main__':
    ...
    dir_path = 'datasets/MOT17_train/front_half/' + sequenceName + "/"
    ...
    for i,frame in enumerate(targetFrame):
        ...
        # 读取的图片路径
        backgroundImagePath = "./datasets/MOT17_train/front_half/" + sequenceName + "/img1/" + "{:0>6d}".format(frame) + ".jpg"
        originalImagePath = backgroundImagePath

        # 保存的图像路径
        saveFilePath = "./results_FairMOT_2/mot17_train_fronthalf/tackling/MattingRepair/" + sequenceName + "/" + "{:0>6d}".format(frame) + ".jpg"
